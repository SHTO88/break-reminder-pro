  ...
  try {
    const newSettings = {
      ...
      break_duration_minutes:
        parseInt(document.getElementById("break-duration-minutes").value) || 0,
      break_duration_seconds:
        parseInt(document.getElementById("break-duration-seconds").value) || 0,
      break_mode:
        document.querySelector('input[name="break-mode"]:checked')?.value ||
        "force",
    ...
    console.log(
      `Break duration saved as: ${newSettings.break_duration_minutes}m ${newSettings.break_duration_seconds}s`,
    );
  } catch (error) {
...
async function startTimer(seconds) {
  console.log(
    `🚀 Starting timer with ${seconds} seconds (${Math.floor(seconds / 60)}:${seconds % 60})`,
  );
...
function resumeTimer() {
  ...
  timerInterval = setInterval(async () => {
    ...
    if (timerSeconds <= 30) {
      console.log(
        `⏰ Timer countdown (resumed): ${timerSeconds} seconds remaining`,
      );
    }
    ...
    if (statusElement) {
      if (isTimerPaused) {
        ...
        statusElement.textContent =
          "Timer is running. Stay focused and productive!";
      }
    }
  } else {
...
function updateInfoPanel() {
  ...
  if (toggleStatusDisplay) {
    ...
    toggleStatusDisplay.className = recurring
...
function getBreakDurationValue() {
  ...
  console.log(
    `Break duration: ${Math.floor(totalSeconds / 60)}m ${totalSeconds % 60}s = ${totalSeconds} total seconds`,
  );
  return totalSeconds;
}
...
async function checkPreBreakTrigger() {
  ...
  try {
    ...
    const preBreakTimingSeconds =
      currentTimerSettings.pre_break_minutes * 60 +
      currentTimerSettings.pre_break_seconds;

    // Trigger pre-break when remaining time equals pre-break timing
    if (timerSeconds === preBreakTimingSeconds) {
      console.log(
        `🚨 PRE-BREAK TRIGGER! ${preBreakTimingSeconds} seconds remaining (should show at bottom center)`,
      );
      ...
      document.getElementById("timer-status").textContent =
        `Pre-break warning shown - break starting in ${timingText}`;
    }
  } catch (error) {
...
async function handleBreakTime() {
  try {
    ...
    const breakMode = document.querySelector(
      'input[name="break-mode"]:checked',
    ).value;
    ...
    if (settings.meeting_detect) {
      ...
      if (meetingReason) {
        console.log(`Meeting detected (${meetingReason}), postponing break...`);
        document.getElementById("timer-status").textContent =
          `Meeting detected (${meetingReason}) - break postponed by 10 minutes`;
        try {
    ...
    document.getElementById("timer-status").textContent =
      "Error triggering break. Please try again.";
  }
}
  ...
  try {
    ...
    switch (breakMode) {
      case "force":
        ...
        document.getElementById("timer-status").textContent =
          `Force break window opened - ${breakDurationSeconds}s break!`;
        break;
      case "notify":
        await invoke("notify_window", { duration: breakDurationSeconds });
        document.getElementById("timer-status").textContent =
          `Break notification shown - ${breakDurationSeconds}s break!`;
        break;
      case "lock":
        await invoke("lock_screen");
        document.getElementById("timer-status").textContent =
          "Screen locked for break time";
        break;
    }

    if (settings.recurring) {
      console.log(
        `🔄 Setting up recurring timer for ${breakDurationSeconds} seconds`,
      );

      // Update status to show break is in progress
      document.getElementById("timer-status").textContent =
        `Break in progress... Next timer starts in ${Math.ceil(breakDurationSeconds / 60)} minutes`;
    ...
    document.getElementById("timer-status").textContent =
      "Error during break. Please check your settings.";
  }
}
...
function handleBreakSkipped() {
  ...
  if (wasRecurring) {
    ...
    if (statusElement) {
      statusElement.textContent =
        "⏭️ Break skipped - Starting next timer session...";
    }
...
function handleEarlyBreakReturn() {
  if (breakEndHandled) {
    console.log(
      "⚠️ handleEarlyBreakReturn() called again — ignoring duplicate",
    );
    return;
  }
  breakEndHandled = true;
  ...
  if (currentTimerSettings && currentTimerSettings.recurring) {
    ...
    if (statusElement) {
      statusElement.textContent =
        "🔄 Break ended early - Starting next timer session...";
    }
    ...
    if (statusElement) {
      statusElement.textContent =
        "✅ Break completed early - Timer session finished";
    }
